...
reveal = response %>% select(-game) %>%
  ...
  arrange(match(symbol, names))  %>% as.data.frame()
...
pf = pf %>% 
  tidyr::pivot_wider(names_from = team, values_from=pfval) %>% select(time, team1, team2, team3, team4, team5, team6, team7, team8, team9, team10) 
```

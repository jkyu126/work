...
1. 1991-1995 닷컴버블 이전 (보여주는 데이터)
2. 1996-2000 닷컴버블 (투자시기 1)
3. 2005-2010 경제위기 (투자시기 2)
4. 2015-2020 회복기 (투자시기 3)
...
response = read.csv("data/response/response1.csv", stringsAsFactors = F)[,-c(1:3)] 
...
response = response %>% mutate(team = paste0("team", as.numeric(substr(team,1,1))), 
  ...
  pivot_longer(-c(team:game), names_to = "symbol", values_to = "pfratio") %>% filter(game==2)
...
pf = mydat %>% 
  ...
  as.data.frame()
write.csv(pf, "data/pf/stage2.csv", row.names=F)
pf = pf %>% select(-game) %>% 

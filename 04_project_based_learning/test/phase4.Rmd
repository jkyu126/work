...
pf = mydat %>% 
  ...
  as.data.frame()
write.csv(pf, "data/pf/stage3.csv", row.names=F)
pf = pf %>% select(-game) %>% 
...
pf1 = read.csv("data/pf/stage1.csv", stringsAsFactors = F)

cum1 = pf1 %>% filter(time=="2025-12-01") %>% 
  mutate(multiplier = pfval) %>%
  select(team, multiplier)

pf2 = read.csv("data/pf/stage2.csv", stringsAsFactors = F) %>% 
...
pf3 = read.csv("data/pf/stage3.csv", stringsAsFactors = F) %>% 

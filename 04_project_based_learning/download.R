...
us_home <-tq_get("USSTHPI", get="economic.data", from=from)
...
us_home = us_home %>% mutate(symbol = "us_home")


DF = rbind(sp500, djia, kospi, vix, oil, gold, usd, us_home, us_bond_10yr, us_tbill_90day)
DF2 = DF %>% 
...
DF4 = DF3 %>%
  ...
  mutate(period = ifelse(1998<=year & year<=2002, "Phase0", 
                         ifelse(2004<=year & year<=2009, "Phase1",
...
DF5 = DF4 %>%
  ...
  mutate(plusyear = ifelse(period=="Phase0", 18, 17)) %>%

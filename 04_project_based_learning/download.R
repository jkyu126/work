...
names = c("djia", "gold", "kospi", "oil", "sp500", "bond_1yr",
          "us_home", "bond_20yr", "usd", "vix")
names2 = data.frame(original = names, Asset = paste("Asset", 1:10), stringsAsFactors = F)

read.csv("data/sample_phase0.csv", stringsAsFactors = F) %>%
  left_join(names2, by=c("symbol"="original")) %>%
  select(Asset, time, price, period) %>%
  write.csv("data/public/phase0.csv", row.names=F)

read.csv("data/sample_phase1.csv", stringsAsFactors = F) %>%
  left_join(names2, by=c("symbol"="original")) %>%
  select(Asset, time, price, period) %>%
  write.csv("data/public/phase1.csv", row.names=F)

read.csv("data/sample_phase2.csv", stringsAsFactors = F) %>%
  left_join(names2, by=c("symbol"="original")) %>%
  select(Asset, time, price, period) %>%
  write.csv("data/public/phase2.csv", row.names=F)

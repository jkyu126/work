...
mydat = read.csv("../data/sample_dataset.csv", stringsAsFactors = F) %>% filter(period %in% c("Phase2"))

response = read.csv("response/response.csv", stringsAsFactors = F)[,-c(1:2)]
colnames(response) = c("class", "team", "game", names)
response = response %>% filter(class=="4월 24일") %>% select(-class)

response = response %>% mutate(team = paste0("team", as.numeric(substr(team,1,2))), 
                               game = as.numeric(substr(game, 6,6))) %>% filter(game==2) %>%
...
# Phase Two Evaluation (데이터 제공: 2026-2030)


```{r, include=FALSE}
price = mydat %>% filter(time=="2026-01-01")
response = response %>% filter(game==2) %>%
...
pf = mydat %>% 
  ...
  as.data.frame()
write.csv(pf, "pf/stage2.csv", row.names=F)
pf = pf %>% 
  tidyr::pivot_wider(names_from = team, values_from=pfval) %>% 
  select(time, team1, team2, team3, team4, team5, team6, team7, team8, team9, team10, team11, team12, team13) 
```

## 팀별 포트폴리오 비율 공개
```{r, echo = F}
reveal %>%
  select(symbol, team1, team2, team3, team4, team5, team6, team7, team8, team9, team10, team11, team12, team13) %>%
  kable() %>%
...
fig <- plot_ly(x = ~time, y = ~eval(parse(text=teams[1])), 
  ...
  layout(title = "Portfolio Performance from 2026-2030", yaxis=list(title="value"), 
...
fig <- fig %>% add_trace(y = ~eval(parse(text=teams[2])), name = paste("Team", 2), mode = 'lines') %>%
  ...
  add_trace(y = ~eval(parse(text=teams[11])), name = paste("Team", 11), mode = 'lines') %>%
  add_trace(y = ~eval(parse(text=teams[12])), name = paste("Team", 12), mode = 'lines') %>%
  add_trace(y = ~eval(parse(text=teams[13])), name = paste("Team", 13), mode = 'lines') 
...
## 자산별 성과공개 Phase Three 포트폴리오 구성 (데이터 제공: 2026-2030)
* 아래 반응형 도표는 2026년부터 2030년까지의 10가지 전세계 자산 가격의 데이터입니다. 
* 각 자산의 가격은 비교를 용이하게 하기 위해 2026년 1월 가격을 1로 표준화하였습니다.
...
* 조별로 토론하고, 2030년 시점에서 어떻게 포트폴리오를 구성할 지 생각하여 제출하세요. 
...
fig <- plot_ly(x = ~time, y = ~eval(parse(text=names[1])), 
               type = "scatter", mode="lines", name="Asset 1", data=phase2) %>%
  layout(title = "Asset Price from 2026-2030", yaxis=list(title="price"), 
         xaxis = list(type = "date", tickformat="%b %Y", tickangle=30))
...
데이터 다운로드: https://drive.google.com/open?id=1L2x03o_AClJlmrmNpoiLsKviJlYkYT8a

...
response = response %>% filter(class=="5월 8일") %>% select(-class)
...
pf = pf %>% 
  ...
  select(time, team1, team2, team3, team4, team5, team6, team7, team8, team9, team10, team11) 
```

## 팀별 포트폴리오 비율 공개
```{r, echo = F}
reveal %>%
  select(symbol, team1, team2, team3, team4, team5, team6, team7, team8, team9, team10, team11) %>%
  kable() %>%
...
pf = pf %>% select(-game) %>% 
  tidyr::pivot_wider(names_from = team, values_from=pfval) %>%
  select(time, team1, team2, team3, team4, team5, team6, team7, team8, team9,
         team10, team11)
